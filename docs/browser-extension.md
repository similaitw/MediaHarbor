...
MediaHarbor Launcher 是一個 WebExtension，可在 Chrome、Microsoft Edge 與 Firefox 使用。它不在瀏覽器裡直接下載影片，而是抓取目前分頁 URL，送到 MediaHarbor Web Console 解析，或複製本機 CLI 指令。
...
4. 點 `Open Web Console`，或點 `Copy CLI` 後到本機終端機執行。
